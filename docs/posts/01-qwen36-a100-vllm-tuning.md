...
整个过程分两个阶段：第一轮是「能跑起来、确定档位」，第二轮是「在 128K 档位上做细粒度参数扫描」。
...
| 篇目 | 主题 | 状态 |
|------|------|------|
| **第 1 篇** | 双 A100 部署与调优 Qwen3.6-27B（本文） | 已发布 |
| 第 2 篇 | 基于该推理基座，构建运营 Agent 的 MVP | 计划中 |
| 第 3 篇 | RAG / 长文档场景下的 prefix 与上下文策略 | 待定 |

**可复现资源**（GitHub `ai-production-lab`，与掘金正文配套）：

- `configs/docker-run-main-128k-r2-final.sh` — 推荐生产启动脚本
- `scripts/benchmark_r2.py` / `run_round2_matrix.sh` — 第二轮基准与矩阵
- `results/summary.csv` — 14 变体实测汇总
