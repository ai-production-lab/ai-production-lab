...
- 14 组配置变体的 benchmark 数据与最终推荐配置（相对 T7b，**512 token 生成吞吐 +20%**）
...
- **第一轮（standard_path）**：64K 部署验收后，自动化跑 12 个阶段，确定 **128K 主力档位** 与 T7b 组合（FP8 KV + prefix + MTP）。
- **第二轮（R2 矩阵）**：在 T7b 基准上做单因子扫描（14 个配置变体），最终推荐 **R2-A1**（BF16 KV + prefix + MTP×2）。
...
14 个配置变体，单因子扫描，每次相对基准（**R2-A0** = T7b）只改一个参数：
...
每个配置变体跑 6 项基准（B1～B6），其中 **B6（2048 token 长生成）** 专门用于判断 MTP 效果——512 token 短测不足以体现 MTP 加速：
...
|---------|------------------------|-------------------|-------------------------|----------------|------------------|
| **R2-A0** 基准 T7b | 70.95 | 7.41 | 73.08 | 39.6% | 5476 |
...
相对初始 T7b（R2-A0）：512 token 生成 **+20%**，长文 prefill **-6%**。
